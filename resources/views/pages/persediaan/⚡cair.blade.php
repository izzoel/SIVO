...
new class extends Component {
    ...
    {
        return Satuan::query()->where('jenis', 'cair')->orderBy('nama')->get();
    }
    ...
    public function import()
    {
        ...
        $this->resetErrorBag('xlsx_file');
        $this->dispatch('modal-close', name: 'modal-cair-masal');
    ...
    public function simpan()
    {
        ...
        $this->dispatch('modal-close', name: 'modal-cair-tambah');
        Flux::toast(variant: 'success', text: __('Data cair berhasil disimpan.'));
    }
    ...
    public function ubah($id)
    {
        ...
        $this->reset(['nama', 'stok', 'spesifikasi', 'id_satuan', 'id_laboratorium', 'id_lokasi', 'id_status']);
        $this->resetValidation();
        $this->dispatch('modal-close', name: 'modal-cair-ubah');
        Flux::toast(variant: 'success', text: __('Data cair berhasil diubah.'));
    }
    ...
    public function hapus()
    {
        ...
        $this->dispatch('modal-close', name: 'modal-cair-hapus');

        Flux::toast(variant: 'success', text: __('Data cair berhasil dihapus.'));
    }
    ...
    public function modalUbah($id)
    {
        ...
        $this->dispatch('modal-show', name: 'modal-cair-ubah');
    }
    ...
    public function modalTambah()
    {
        ...
        $this->reset(['nama', 'stok', 'spesifikasi', 'id_satuan', 'id_laboratorium', 'id_lokasi', 'id_status']);
        $this->resetValidation();
        $this->dispatch('modal-show', name: 'modal-cair-tambah');
    }
    ...
    public function modalHapus($id)
    {
        ...
        $this->dispatch('modal-show', name: 'modal-cair-hapus');
    }
...
<div class="space-y-6">
    <x-pages-navbar>
        <flux:breadcrumbs.item :href="route('persediaan.cair')"> {{ __('Persediaan') }} </flux:breadcrumbs.item>

        <flux:breadcrumbs.item :href="route('persediaan.cair')" :current="request()->routeIs('persediaan.cair')"> {{ __('Cair') }} </flux:breadcrumbs.item>
    </x-pages-navbar>

    <div class="space-y-2">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <flux:heading class="mb-0!" size="xl">{{ __('Cair') }}</flux:heading>
                <flux:button wire:click="modalTambah" size="xs" variant="primary" icon="plus"></flux:button>
                <flux:modal.trigger name="modal-cair-masal">
                    <flux:button size="xs" variant="primary" color="indigo" icon="document-plus"></flux:button>
                </flux:modal.trigger>
            </div>
        </div>

        <flux:text class="text-zinc-600 dark:text-zinc-400"> {{ __('Kelola data persediaan cair di sini.') }} </flux:text>
    </div>
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div class="overflow-x-auto mt-3">
            <flux:table :paginate="$this->cair">
                ...
                <flux:table.rows>
                    @forelse ($this->cair as $cair)
                        <flux:table.row :key="$cair->id">
                            <flux:table.cell class="whitespace-nowrap">{{ $cair->nama }}</flux:table.cell>
                            <flux:table.cell class="whitespace-nowrap">
                                <flux:badge color="lime">{{ $cair->stok }}</flux:badge>
                                <x-text-cell text="{{$cair->satuan?->nama}}" warna="{{$cair->satuan?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>{{ $cair->spesifikasi }}</flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$cair->laboratorium?->nama}}" warna="{{$cair->laboratorium?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$cair->lokasi?->nama}}" warna="{{$cair->lokasi?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$cair->statusKerusakan?->nama ?? '-'}}" warna="{{$cair->statusKerusakan?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <flux:dropdown position="bottom" align="end">
                                    <flux:button variant="ghost" size="sm" icon="ellipsis-horizontal" inset="top bottom" />
                                    <flux:menu>
                                        <flux:menu.item wire:click="modalUbah({{ $cair->id }})" icon="pencil-square"> Edit </flux:menu.item>

                                        <flux:menu.item wire:click="modalHapus({{ $cair->id }})" icon="trash" variant="danger"> Hapus </flux:menu.item>
                                    </flux:menu>
                                </flux:dropdown>
                            </flux:table.cell>
                        </flux:table.row>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-zinc-500">Belum ada data cair.</td>
                        </tr>
                    @endforelse
                </flux:table.rows>
            </flux:table>
    ...
    <x-modal name="modal-cair-tambah" mode="tambah" title="{{ __('Cair') }}" description="{{ __('Menambahkan persediaan cair baru') }}" maxWidth="2xl" color="lime" klik="simpan">
        <div class="grid grid-cols-4 gap-4">
            <div class="col-span-2">
                <flux:input wire:model="nama" label="Nama" placeholder="nama persediaam" />
            </div>
    ...
    <x-modal name="modal-cair-ubah" mode="ubah" title="{{ __('Cair') }}" description="Ubah data cair." maxWidth="2xl" color="amber" klik="ubah({{ $editId }})">
        <div class="grid grid-cols-4 gap-4">
            <div class="col-span-2">
    ...
    <x-modal name="modal-cair-hapus" mode="hapus" title="{{ __('Cair') }}" description="Yakin ingin menghapus data ini?" maxWidth="lg" color="red" klik="hapus">
