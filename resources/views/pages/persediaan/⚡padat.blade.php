...
new class extends Component {
    ...
    {
        return [
            'nama.required' => 'Tulis nama padat.',
    ...
    {
        return Satuan::query()->where('jenis', 'padat')->orderBy('nama')->get();
    }
    ...
    public function import()
    {
        ...
        $this->resetErrorBag('xlsx_file');
        $this->dispatch('modal-close', name: 'modal-padat-masal');
    ...
    public function simpan()
    {
        ...
        $this->dispatch('modal-close', name: 'modal-padat-tambah');
        Flux::toast(variant: 'success', text: __('Data padat berhasil disimpan.'));
    }
    ...
    public function ubah($id)
    {
        ...
        $this->reset(['nama', 'stok', 'spesifikasi', 'id_satuan', 'id_laboratorium', 'id_lokasi', 'id_status']);
        $this->resetValidation();
        $this->dispatch('modal-close', name: 'modal-padat-ubah');
    ...
    public function hapus()
    {
        ...
        $this->dispatch('modal-close', name: 'modal-padat-hapus');

        Flux::toast(variant: 'success', text: __('Data padat berhasil dihapus.'));
    }
    ...
    public function modalUbah($id)
    {
        ...
        $this->dispatch('modal-show', name: 'modal-padat-ubah');
    }
    ...
    public function modalTambah()
    {
        ...
        $this->reset(['nama', 'stok', 'spesifikasi', 'id_satuan', 'id_laboratorium', 'id_lokasi', 'id_status']);
        $this->resetValidation();
        $this->dispatch('modal-show', name: 'modal-padat-tambah');
    }
    ...
    public function modalHapus($id)
    {
        ...
        $this->dispatch('modal-show', name: 'modal-padat-hapus');
    }
...
<div class="space-y-6">
    <x-pages-navbar>
        <flux:breadcrumbs.item :href="route('persediaan.padat')"> {{ __('Persediaan') }} </flux:breadcrumbs.item>
        <flux:breadcrumbs.item :href="route('persediaan.padat')" :current="request()->routeIs('persediaan.padat')"> {{ __('Padat') }} </flux:breadcrumbs.item>
    </x-pages-navbar>

    <div class="space-y-2">
        <div class="flex items-center justify-between">
            <div class="flex items-center gap-2">
                <flux:heading class="mb-0!" size="xl">{{ __('Padat') }}</flux:heading>
                <flux:button wire:click="modalTambah" size="xs" variant="primary" icon="plus"></flux:button>
                <flux:modal.trigger name="modal-padat-masal">
                    <flux:button size="xs" variant="primary" color="indigo" icon="document-plus"></flux:button>
                </flux:modal.trigger>
            </div>
        </div>

        <flux:text class="text-zinc-600 dark:text-zinc-400"> {{ __('Kelola data persediaan padat di sini.') }} </flux:text>
    </div>
    ...
    <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <div class="overflow-x-auto mt-3">
            <flux:table :paginate="$this->padat">
                <flux:table.columns>
                ...
                <flux:table.rows>
                    @forelse ($this->padat as $padat)
                        <flux:table.row :key="$padat->id">
                            <flux:table.cell class="whitespace-nowrap">{{ $padat->nama }}</flux:table.cell>
                            <flux:table.cell class="whitespace-nowrap">
                                <flux:badge color="lime">{{ $padat->stok }}</flux:badge>
                                <x-text-cell text="{{$padat->satuan?->nama}}" warna="{{$padat->satuan?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>{{ $padat->spesifikasi }}</flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$padat->laboratorium?->nama}}" warna="{{$padat->laboratorium?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$padat->lokasi?->nama}}" warna="{{$padat->lokasi?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <x-text-cell text="{{$padat->statusKerusakan?->nama ?? '-'}}" warna="{{$padat->statusKerusakan?->warna}}" />
                            </flux:table.cell>
                            <flux:table.cell>
                                <flux:dropdown position="bottom" align="end">
                                    <flux:button variant="ghost" size="sm" icon="ellipsis-horizontal" inset="top bottom" />
                                    <flux:menu>
                                        <flux:menu.item wire:click="modalUbah({{ $padat->id }})" icon="pencil-square"> Edit </flux:menu.item>

                                        <flux:menu.item wire:click="modalHapus({{ $padat->id }})" icon="trash" variant="danger"> Hapus </flux:menu.item>
                                    </flux:menu>
                                </flux:dropdown>
                            </flux:table.cell>
                        </flux:table.row>
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-6 text-center text-zinc-500">Belum ada data padat.</td>
                        </tr>
                    @endforelse
                </flux:table.rows>
            </flux:table>
    ...
    <x-modal
        name="modal-padat-tambah"
        mode="tambah"
        title="{{ __('Padat') }}"
        description="{{ __('Menambahkan persediaan padat baru') }}"
        maxWidth="2xl"
        color="lime"
        klik="simpan"
    >
    ...
    <x-modal name="modal-padat-ubah" mode="ubah" title="{{ __('Padat') }}" description="Ubah data padat." maxWidth="2xl" color="amber" klik="ubah({{ $editId }})">
        <div class="grid grid-cols-4 gap-4">
            <div class="col-span-2">
                <flux:input wire:model="nama" label="Nama" placeholder="nama alat" />
            </div>
    ...
    <x-modal name="modal-padat-hapus" mode="hapus" title="{{ __('Padat') }}" description="Yakin ingin menghapus data ini?" maxWidth="lg" color="red" klik="hapus">
